"""Normalize ALL product images to uniform PNG 1000x1000 on white (no crop).
Flattens any bg onto white, trims near-white margin (tol 248), contain-fits.
Usage: python scripts/normalize-all-v3.py
"""
...
for p in glob.glob("public/images/products/*.*"):
    ...
    if out.lower() != p.lower():
        os.remove(p)
    print("ok", os.path.basename(out), c.size)
