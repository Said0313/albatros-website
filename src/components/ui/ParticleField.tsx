...
// Site-wide particle field, mounted once in the [locale] layout. Client-only:
// it draws on canvas from mount and must never run during SSR. The fixed z-0
// canvas sits behind all routes; content stacks above via z-1 on main.
...
export function ParticleFieldProvider({ children }: { children: React.ReactNode }) {
  ...
  return (
    <FieldRevealContext.Provider value={reveal}>
      <ParticleBackground onRevealReady={onReady} />
      {children}
