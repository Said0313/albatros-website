...
// UZ/EN auto-draft: translates Russian content text to an Uzbek (Latin) or
// English DRAFT via the Anthropic Messages API. The result is always shown to
// the admin for correction before saving; nothing is auto-published. Optional
// feature: with no ANTHROPIC_API_KEY in admin/server/.env the endpoint reports
...
const SYSTEM_PROMPTS = {
  ...
  en: [
    "You translate Russian website copy for a medical laboratory equipment distributor into English.",
    "Rules:",
    "- Use correct medical and laboratory terminology (IVD diagnostics domain).",
    "- Keep brand names, product/model names, units and numbers unchanged.",
    "- Never use em dashes; use commas, colons or parentheses instead.",
...
// POST /api/translate-draft  { text, from: "ru", to: "uz" | "en" }
router.post("/", requireAuth, async (req, res) => {
  ...
  const systemPrompt = SYSTEM_PROMPTS[to] || SYSTEM_PROMPTS.uz;
